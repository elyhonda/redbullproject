...
public class PlayerMovement : MonoBehaviour
{
    ...
    void FixedUpdate()  //world movement
    {
        ...
        float CamVel = 0;
        if (Rigid != null)
            CamVel = Rigid.linearVelocity.magnitude;
        ...
        else if (States == WorldState.InAir)
        {
            ...
            //falling audio
            Visuals.WindAudioSetting(delta, Rigid.linearVelocity.magnitude);
        ...
        else if (States == WorldState.Flying)
        {
            ...
            //lerp speed
            float YAmt = Rigid.linearVelocity.y;
        ...
        else if (States == WorldState.Stunned)
        {
            ...
            FallDir.y = Rigid.linearVelocity.y;
            Rigid.linearVelocity = Vector3.Lerp(Rigid.linearVelocity, FallDir, delta * 2f);
    ...
    void AnimCtrl()
    {
        ...
        Vector3 RelVel = RelPos.transform.InverseTransformDirection(Rigid.linearVelocity);
        ...
        Anim.SetFloat("YVel", Rigid.linearVelocity.y);

        //set movement animator
        RunTimer = new Vector3(Rigid.linearVelocity.x, 0, Rigid.linearVelocity.z).magnitude;
    ...
    void FallingCtrl(float d, float Speed, float Accel, Vector3 moveDirection)
    {
        ...
        if(Rigid.linearVelocity.y < -6) //we are going downwards
        {
            LerpDirection = Vector3.up;
            FallDirSpd = FallDirSpd * -(Rigid.linearVelocity.y * 0.2f);
        }         
    ...
    void FlyingCtrl(float d, float Speed, float XMove, float ZMove)
    {
        ...
        //lerp velocity
        Vector3 dir = Vector3.Lerp(Rigid.linearVelocity, targetVelocity, d * FlyLerpSpd);
        Rigid.linearVelocity = dir;
    }
    ...
    {
        Quaternion SlerpRot = Quaternion.LookRotation(Rigid.linearVelocity.normalized);
        transform.rotation = Quaternion.Slerp(transform.rotation, SlerpRot, spd * d);
    }
